...
**实现方式：**
组合继承（Combination Inheritance）结合了原型链继承和构造函数继承的优点，通过调用父构造函数继承属性，通过原型链继承方法。
...
function Child(name, age) {
  // 继承属性 - 第二次调用Parent构造函数
  Parent.call(this, name);
  this.age = age;
}

// 继承方法 - 第一次调用Parent构造函数
Child.prototype = new Parent();
// 修复constructor指向
Child.prototype.constructor = Child;
...
- 父构造函数被调用两次（一次在创建子原型时，一次在子构造函数内部）
- 子原型上会存在父构造函数的实例属性，造成内存浪费

### 4. 寄生组合继承

**实现方式：**
寄生组合继承（Parasitic Combination Inheritance）通过借用构造函数继承属性，通过原型链的混成形式继承方法，解决了组合继承中父构造函数被调用两次的问题。
...
function Child(name, age) {
  // 继承属性 - 仅调用一次Parent构造函数
  Parent.call(this, name);
  this.age = age;
}

// 关键优化：使用Object.create创建父原型的副本，避免调用Parent构造函数
...
1. **减少构造函数调用次数**
   - 组合继承：父构造函数被调用两次（`new Parent()`和`Parent.call()`）
   - 寄生组合继承：父构造函数仅被调用一次（仅`Parent.call()`）

2. **避免原型对象中的冗余属性**
   - 组合继承：子原型对象上会存在父构造函数的实例属性
   - 寄生组合继承：子原型仅包含父原型的方法，没有多余属性

3. **内存效率更高**
   - 避免在原型链上创建不必要的属性副本
   - 减少内存占用，尤其在创建大量实例时优势明显
...
> **最佳实践**：在ES6之前，寄生组合继承被认为是实现继承的最优方案。许多JavaScript库和框架（如jQuery、React早期版本）都采用这种继承方式。ES6的class语法糖底层也是基于类似的机制实现的。
